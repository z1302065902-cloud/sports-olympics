// ============================================================
// game.js — 奥运50关 · SPORTS ADVENTURE 主引擎
...
class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(W / 2, H * 0.24, '🏅 OLYMPIC 50 LEVELS', { fontSize: 56, fontStyle: 'bold', color: '#ffffff' })
      .setOrigin(0.5).setStroke('#7a4a9a', 8).setShadow(0, 4, '#00000055', 6);
    this.add.text(W / 2, H * 0.34, 'SPORTS ADVENTURE', { fontSize: 30, color: '#ffe66d' }).setOrigin(0.5)
      .setStroke('#7a4a9a', 6);
    this.add.text(W / 2, H * 0.43, '50 SPORTS EVENTS', { fontSize: 18, color: '#ffffff' }).setOrigin(0.5);
    const done = Object.values(this.prog).filter(v => v >= 2).length;
    const stars = Object.values(this.prog).reduce((a, v) => a + (v === 3 ? 1 : 0), 0);
    this.add.text(W / 2, H * 0.5, `COMPLETED ${done}/50 · ⭐${stars}`, { fontSize: 20, color: '#aee3ff' }).setOrigin(0.5);
    const btn = this.add.graphics().fillStyle(0xff6b35, 1).fillRoundedRect(W / 2 - 100, H * 0.6, 200, 60, 16);
    this.add.text(W / 2, H * 0.63, 'START', { fontSize: 26, fontStyle: 'bold', color: '#ffffff' }).setOrigin(0.5);
    ...
    this.add.text(W / 2, H * 0.75, '🖱️ MOUSE / 📱 TOUCH CONTROLS', { fontSize: 16, color: '#ffffff' }).setOrigin(0.5);
    this.add.text(W / 2, H * 0.8, 'HOLD for power · TAP FAST for combos · TIME your clicks', { fontSize: 14, color: '#c5e8ff' }).setOrigin(0.5);
  }
}
...
class MapScene extends Phaser.Scene {
  ...
  create() {
    ...
    new BgWorld(this, 'stadium', 0);
    const W = this.scale.width, H = this.scale.height;
    this.add.text(W / 2, 40, 'SELECT LEVEL', { fontSize: 32, fontStyle: 'bold', color: '#ffffff' })
      .setOrigin(0.5).setStroke('#7a4a9a', 6);
    this.add.text(W / 2, 70, '🏅 = Gold · 🥈 Silver · 🥉 Bronze', { fontSize: 14, color: '#ffe66d' }).setOrigin(0.5);
    ...
    const back = this.add.text(40, 32, '◀ MENU', { fontSize: 18, color: '#ffffff' }).setInteractive({ useHandCursor: true });
...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    const W = this.scale.width, H = this.scale.height;
    this.add.text(W / 2, 30, `${lv.emoji} Level ${lv.name}`, { fontSize: 26, fontStyle: 'bold', color: '#ffffff' })
    ...
    const back = this.add.text(34, 28, '◀ LEVELS', { fontSize: 16, color: '#ffffff' }).setInteractive({ useHandCursor: true });
  ...
  initTiming(W, H) {
    ...
    this.scoreText = this.add.text(W / 2, H * 0.7, 'Click to Stop!', { fontSize: 18, color: '#ffe66d' }).setOrigin(0.5);
  ...
  initPowerStage2() {
    ...
    this.add.text(W / 2, H * 0.48, 'JUMP', { fontSize: 18, color: '#ffffff' }).setOrigin(0.5);
    this.add.text(W / 2, H * 0.22, 'Jump Power!', { fontSize: 20, color: '#ffe66d' }).setOrigin(0.5);
  ...
  initPowerAngle(W, H) {
    ...
    this.add.text(W * 0.75, H * 0.65, 'CHARGE', { fontSize: 16, color: '#ffffff' }).setOrigin(0.5);
  ...
  initComboTiming(W, H) {
    ...
    this.scoreText = this.add.text(W / 2, H * 0.4, 'Tap to Swim!', { fontSize: 24, color: '#ffe66d' }).setOrigin(0.5);
    this.timeText = this.add.text(W / 2, H * 0.48, `Time: ${this.timeLeft}s`, { fontSize: 18, color: '#ffffff' }).setOrigin(0.5);
    ...
    this.zone.on('pointerdown', () => {
      ...
      if (!this.phase) { this.phase = 'go'; SFX.play('go');
        this.countdownTimer = this.time.addEvent({ delay: 1000, loop: true, callback: () => {
          this.timeLeft--; this.timeText.setText(`Time: ${this.timeLeft}s`); SFX.play('count');
  ...
  initCombo(W, H) {
    ...
    this.timeText = this.add.text(W / 2, H * 0.62, `Time: ${this.timeLeft}s`, { fontSize: 18, color: '#ffffff' }).setOrigin(0.5);
...
class ResultScene extends Phaser.Scene {
  ...
  create() {
    ...
    mkBtn(H * 0.68, index < 49 ? 'NEXT ▶' : 'COMPLETE 🎉', () => {
      if (index < 49) { SFX.unlock(); SFX.startBgm((index + 1) % 6); this.scene.start('game', { level: LEVELS[index + 1], index: index + 1 }); }
      else this.scene.start('menu');
    });
    mkBtn(H * 0.78, 'REPLAY', () => { SFX.unlock(); SFX.startBgm(index % 6); this.scene.start('game', { level: lv, index }); });
    mkBtn(H * 0.88, 'BACK', () => { SFX.unlock(); SFX.startBgm(0); this.scene.start('map'); });
  }
}
